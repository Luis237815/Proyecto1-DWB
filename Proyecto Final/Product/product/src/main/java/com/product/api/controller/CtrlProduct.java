...
public class CtrlProduct {
	...
	@GetMapping("/{gtin}")
	public ResponseEntity<Product> getProduct(@PathVariable("gtin") @Valid String gtin){
	...
	public ResponseEntity<ApiResponse> updateProductStock(@PathVariable("stock") Integer stock, @PathVariable("gtin") @Valid String gtin){
	...
	public ResponseEntity<ApiResponse> updateProductCategory(@PathVariable("gtin") String gtin, @Valid @RequestBody DtoProductCategory category_id, BindingResult bindingResult){
		if(bindingResult.hasErrors())
			throw new ApiException(HttpStatus.BAD_REQUEST, bindingResult.getAllErrors().get(0).getDefaultMessage());
